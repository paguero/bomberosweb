...
onMounted(async () => {
  ...
  storeCotizacion.getEmisionValidacion(
    {
      numeroFolio:storeCotizacion.currentCotizacion.numeroFolio,
      numeroPoliza:storeCotizacion.currentCotizacion.numeroPoliza,
      patente:storeCotizacion.currentCotizacion.vehiculo.patente}).then(()=>{
        cotizacionDetails.value = storeCotizacion.currentCotizacion;
        cotizacionDetails.value.fechaInicio =  moment(storeCotizacion.currentCotizacion.fechaInicio, "YYYYMMDD hh:mm:ss").toDate();
        cotizacionDetails.value.fechaTermino =  moment(storeCotizacion.currentCotizacion.fechaTermino, "YYYYMMDD hh:mm:ss").toDate();
      }).catch(() => {
          const [error] = Object.values(storeCotizacion.cotizacionErrors);
          Swal.fire({
            text: error,
